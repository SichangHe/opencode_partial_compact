...
`40-kv-cache-strategy.md` follows persistence in this read order because it
depends on how the message view is rewritten.
...
- POC at [`../experiments/poc/`](../experiments/poc/) — registers a
  ...
  tool visible to the model. `/tmp/pc-poc.log` shows the run. Current
  hook names and SDK client calls are type/build checked against the
  installed 1.15.x plugin/SDK packages.
