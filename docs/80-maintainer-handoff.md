...
- Agent tools are `partial_compact` and `partial_compact_instructions`. `partial_compact` accepts `ranges: [{ session_id?, from_message_id, to_message_id, summary }]`; omit `session_id` for the current session. The instruction tool appends ordered current-session `msg...` IDs for range selection.
...
- Native compaction is blocked with `experimental.session.compacting`; if a native compaction path still escapes, `experimental.compaction.autocontinue` disables synthetic continuation and later transforms perform damage-control pruning for visible native compaction parts.
...
The plugin cannot rewrite a previous assistant message's already-recorded token usage. Opencode checks that record before plugins can recompute the partial-compacted effective context. To avoid stale-trigger native compactions when the visible context is already small, the plugin enforces `compaction.auto=false` in Opencode's merged runtime config and blocks native compaction hooks as a fail-closed guard. Every subsequent normal model call uses the `messages.transform` output, i.e. the partial-compacted effective context; native/full compaction is refused while the plugin is enabled.
