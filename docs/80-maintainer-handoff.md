...
- Agent tools are `partial_compact`, `partial_compact_instructions`, and `partial_compact_current_session_message_ids`. `partial_compact` accepts current-session message ranges: `ranges: [{ from_message_id, to_message_id, summary }]`. The instruction tool returns only policy; the message-ID tool returns ordered current-session `msg...` IDs for range selection.
- TUI registers `/partial_compact` and `/partial-compact` and emits a one-shot prompt with an instruction-tool pointer, exact selected IDs, and guidance to use one `ranges` call if additional disjoint stale ranges are found.
...
- `experimental.chat.system.transform` injects a mandatory `partial_compact` reminder after estimated visible context growth. It injects only a compact status line like `current context window: 42k (37% full)`. The configured 16k interval is the target cadence; known smaller effective input/context budgets clamp to an internal ~80% safety interval.
