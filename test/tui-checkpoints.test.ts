...
describe("partial compact TUI checkpoints", () => {
  ...
  it("builds an agent prompt that fixes the compaction range", () => {
    ...
    expect(prompt).toContain("call `partial_compact` with one `ranges` item")
    expect(prompt).toContain("partial_compact_instructions")
    expect(prompt).not.toContain("<instruction name=\"opencode-partial-compact\">")
    expect(prompt).toContain("additional items in the same `ranges` call")
  })
})
